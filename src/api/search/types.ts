...
// 博文列表信息
export interface GroupBlogItemInterface {
  id: number | string;
  apartmentId: number | string;
  userId: number | string;
  // 标题
...
// 博文点赞列表
export interface BlogLikeItemInterface {
  // 用户id
  id: number | string;
  nickname: string;
  avatarUrl: string;
...
// 博文评论列表
export interface BlogCommentItemInterface {
  id: number | string;
  userId: number | string;
  ...
  // 子评论列表（嵌套结构）
  children?: BlogCommentItemInterface[];
}

// 关注用户信息
export interface GroupFollowInterface {
  // 用户id
  id: number;
  // 关注的用户id
  followUserId: number;
